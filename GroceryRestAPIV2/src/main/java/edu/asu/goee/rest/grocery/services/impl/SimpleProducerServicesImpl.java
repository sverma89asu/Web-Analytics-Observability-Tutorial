package edu.asu.goee.rest.grocery.services.impl;
...
public class SimpleProducerServicesImpl implements ProducerServices {
	...
	@Override
	public Producer findOne(String abb) throws NoSuchProducerException {
		Producer producer = __producers.get(abb);
		if (producer == null) {
			throw new NoSuchProducerException(abb);
		}
		return producer;
	}
	...
	@Override
	public boolean delete(String id) throws NoSuchProducerException {
		boolean rval = __producers.containsKey(id);
		if (rval) {
			__producers.remove(id);
		}
		else {
			throw new NoSuchProducerException(id);
		}
		return rval;
	}
